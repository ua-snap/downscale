...
class DownscalingUtils( object ):
	...
	def interpolate_anomalies( self, anom_df, meshgrid_tuple, template_raster_fn, lons_pcll, \
								src_transform, src_crs, src_nodata, output_filename, *args, **kwargs ):
		'''
		...
		return self.write_gtiff( output_arr, template_meta, output_filename, compress=True )
...
class DownscaleCRU( object ):
	...
	def __init__( self, cru_ts, clim_path, template_raster_fn, base_path, climatology_begin='1961', climatology_end='1990', ncores=2, \
					absolute=True, metric='metric', variable=None, post_downscale_function=None, src_crs={'init':'epsg:4326'}, output_anomalies=True, *args, **kwargs ):
		self.cru_ts = cru_ts
