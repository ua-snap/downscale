...
def tfg_days( x, err='off' ):
    ...
    nodata = -9999 # hardwired nodata
    x[ x == 0 ] = -0.0001 # need to treat zero as freezing (working with signs)
    mask = (x == nodata)
    
    # positive or negative monthly temps -- at actual data locations
    s1 = np.sign( x, where=~mask )
    s1[ s1 == 0 ] = np.nan # set all the nodata to np.nan
    ...
    for i,j in np.ndindex(x.shape[-2:]):
        idx, = np.where( s[:,i,j] < 0)
        idx = np.sort( np.concatenate( [idx, idx+1] ) )
        out[:len(idx),i,j] = sorted(idx)
...
def run( x ):
    ...
    out_arr = np.full((3,rows,cols), -9999 )

    ind = [ (i,j) for i,j in np.ndindex(arr.shape[-2:]) if not arr[0,i,j] == -9999 ]
    
    vals = [tfg_days( arr[:,i,j] ) for i,j in ind ]
    out_arr[:,i,j]
...
if __name__ == '__main__':
    ...
    ncpus = 64
    models = ['GFDL-CM3','GISS-E2-R','IPSL-CM5A-LR','MRI-CGCM3','NCAR-CCSM4',] # '5ModelAvg', <-- check this
    # models = ['CRU-TS40',]
