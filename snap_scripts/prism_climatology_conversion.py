...
# output_path = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism'
# groups = ['min_temp', 'max_temp']

# # # STEP 1 -- CONVERT TO GTIFF FROM ASC AND TXT

# list the data we want
input_path = '/Data/Base_Data/Climate/AK_CAN_2km/historical/singleBand/prism/AK_CAN_2km_PRISM/CAN_originals/older' #'/Data/Base_Data/Climate/AK_CAN_2km/historical/singleBand/prism/AK_2KM_PRISM/Temperature/2km/older'
output_path = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism'
files = glob.glob( os.path.join( input_path, '*tmin*.asc' ) )
ext = files[0].split('.')[1]
...
# list the data
caw = sorted( glob.glob( os.path.join( '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism/tmax/raw_converted', 'caw*.tif' ) ) )
ak = sorted( glob.glob( os.path.join( '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism/tmax/raw_converted', 'ak*.tif' ) ) )
grouped = zip( ak, caw )
...
# TEMPLATE:
rst = rasterio.open( '/Data/Base_Data/Climate/AK_CAN_2km/projected/AR4_CMIP3_models/sresa1b/cccma-cgcm3-1-t47/tas/tas_mean_C_ar4_cccma_cgcm3_1_sresa1b_01_2001.tif' )
mask = rst.read_mask()
variable = 'tmin'

input_dir = os.path.join( '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism', variable )
...
for fn in files:
	...
	new_path = os.path.join( '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/prism', variable, 'prepped' )
