...
tmp = raster("/big_storage/malindgren/AIEM/RSDS/rad_1km_AKCanada_01_WGS_TEMPLATE.tif")
...
for (m in 1:12) {  # month loop
    ...
    rm(j)
       # Write the output data to a geotiff
        writeRaster(N,filename=paste("rad_1km_wgs84greenwich_akcan_",month[m],'.tif',sep=''),format = 'GTiff',options='COMPRESS=LZW',datatype='FLT4S',overwrite=T)
        rm(N)
