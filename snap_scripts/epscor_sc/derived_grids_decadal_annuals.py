...
if __name__ == '__main__':
	...
	files = glob.glob( os.path.join( base_dir, 'derived_grids','annuals', 
						model, scenario, variable, '*.tif' ) )

	# split to decades
	decades = [ get_decade( fn ) for fn in files ]
	years = [ get_monyear( fn )[1] for fn in files ]
	df = pd.DataFrame( {'decades':decades, 'files':files, 'years':years} )
	file_groups = [ sub_df for idx,sub_df in df.groupby( df.decades ) if len(sub_df.years.unique()) == 10 ]
	...
	for decade, filenames, arr in agg_groups:
		...
		variable, metric, units, project, model, scenario, year = basename.split( '_' )
		...
		out_fn = os.path.join( dirname.replace( 'annuals', 'decadal_annuals' ), new_basename )
