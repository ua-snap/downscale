...
if __name__ == '__main__':
	...
	base_dir = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/downscaled_test_ipsl'
	os.chdir( base_dir )
	scenario = 'rcp85'
	...
	variables_list = [['tasmax', 'tas', 'tasmin']] #[['tasmax', 'tas', 'tasmin'],['pr']]
	models = ['IPSL-CM5A-LR']
	...
	for variables in variables_list:
		for m in models:
			...
			# raw
			plot_df_raw = pd.DataFrame( out_raw )
			plot_df_raw.index = pd.date_range( start=str(begin), end=str(end+1), freq='M' )
			col_list = ['tasmax', 'tas', 'tasmin']
			plot_df_raw = plot_df_raw[ col_list ] # get em in the order for plotting

			# anom
			plot_df_anom = pd.DataFrame( out_anom )
			plot_df_anom.index = pd.date_range( start=str(begin), end=str(end+1), freq='M' )
			col_list = ['tasmax', 'tas', 'tasmin']
			plot_df_anom = plot_df_anom[ col_list ] # get em in the order for plotting
			...
			# now plot the dataframe -- raw
			if begin == end:
				title = 'EPSCoR SC AOI Temp Mean 2KM {} {} {}'.format( m, scenario, begin )
			else:
				title = 'EPSCoR SC AOI Temp Mean 2KM {} {} {} - {}'.format( m, scenario, begin, end )
			...
			ax = plot_df_raw.plot( kind='line', title=title, figsize=figsize, color=colors )

			output_dir = os.path.join( base_dir,'downscaled_test_ipsl','test_min_max_issue' )
			...
			if begin == end:
				output_filename = os.path.join( output_dir,'mean_{}_epscor_sc_{}_{}_{}_2km.png'.format( out_metric_fn, m, scenario, begin ) )
			else:
				output_filename = os.path.join( output_dir,'mean_{}_epscor_sc_{}_{}_{}_{}_2km.png'.format( out_metric_fn, m, scenario, begin, end ) )
			plt.savefig( output_filename, dpi=700 )
			plt.close()



			# now plot the dataframe -- anom
			if begin == end:
				title = 'EPSCoR SC AOI Temp Mean Anomalies 2KM {} {} {}'.format( m, scenario, begin )
			else:
				title = 'EPSCoR SC AOI Temp Mean Anomalies 2KM {} {} {} - {}'.format( m, scenario, begin, end )
			...
			ax = plot_df_anom.plot( kind='line', title=title, figsize=figsize, color=colors )
			...
			if begin == end:
				output_filename = os.path.join( output_dir,'mean_{}_epscor_sc_{}_{}_{}_anom_2km.png'.format( out_metric_fn, m, scenario, begin ) )
			else:
				output_filename = os.path.join( output_dir,'mean_{}_epscor_sc_{}_{}_{}_{}_anom_2km.png'.format( out_metric_fn, m, scenario, begin, end ) )
			plt.savefig( output_filename, dpi=700 )
			plt.close()
