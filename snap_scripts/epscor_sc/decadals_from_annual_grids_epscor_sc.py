def sort_files_annuals( files, split_on='_', elem_year=-1 ):
    '''
...
def only_years_annuals( files, begin=1901, end=2100, split_on='_', elem_year=-1 ):
    '''
...
def make_grouper_df_annuals( files ):
    '''
    ...
    columns = [ 'files', 'years', 'decades' ]
    dat = {'years':years, 'decades':decades, 'files':files}
...
def _aggfun( group, out_fn, metric='mean', fn_col='files', decimals=1 ):
    ...
    agg = metric_switch[ metric ](np.array([ rasterio.open( fn ).read( 1 ) for fn in files ]), axis=0 )
...
if __name__ == '__main__':
    ...
    base_dir = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/derived_grids_FINAL_OCT/annuals'
    output_dir = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/derived_grids_FINAL_OCT/decadal_annuals'
    ...
    for model, scenario, variable in itertools.product( models, scenarios, variables ):
        ...
        files = sort_files_annuals( only_years_annuals( list_files( base_path ), begin, end ) )
        df = make_grouper_df_annuals( files )
