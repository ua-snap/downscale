...
if __name__ == '__main__':
	...
	# shp_fn = '/Data/Base_Data/GIS/GIS_Data/Vector/Boundaries/Alaska_Albers_ESRI.shp'
	...
	# slices = [ ( i, int(str(i)[:3]+'9') ) for i in range( 2010, 2100, 10 )]
	slices = [ (2006, 2100) ]
	...
	for variables in variables_list:
		for m in models:
			for begin, end in slices:
				...
				# THIS IS A THING TO LOOK AT A SINGLE MONTH FOR TOM...
				df = plot_df.reset_index()
				df = df.groupby( df['index'].apply( lambda x: x.month ) ).get_group( 12 )
				df.index = df['index']
				_ = df.pop( 'index' )
				plot_df = df
				# END DATE HACK FOR TOM
				...
				if begin == end:
					title = 'AKCAN Temp Metrics {} {} {}'.format( m, scenario, begin )
				else:
					title = 'AKCAN Temp Metrics {} {} {} - {}'.format( m, scenario, begin, end )
				...
				output_dir = os.path.join( base_dir, 'compare_downscaling_versions_AKCAN', 'DECEMBER_TEMPS' )
