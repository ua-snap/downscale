...
def group_input_filenames( fn_prefix_filter, input_dir ):
	...
	def find_files( input_dir, patterns ):
		"""
		Returns a generator yielding files matching the given patterns
		:type input_dir: str
		:type patterns: [str]
		:rtype : [str]
		:param input_dir: Directory to search for files/directories under. Defaults to current dir.
		...
		for input_dir, dir_names, file_names in os.walk( path ):
	...
	def version_grouper( x ):
		...
		version = [ x for x in input_dir.split( os.path.sep ) if x.startswith( 'v' ) ]
	...
	def drop_old_versions( df ):
		...
		if rows > 1 & rows < 3:
			...
			return df
	# get all matches with fn_prefix_filter
	matches = pd.Series([ match for match in find_files( input_dir, [ fn_prefix_filter ] ) ])
	# group by version if there is a version in the filepath [HARDWIRED!]
	# this is to deal with the nonstandard download and storage of the clt data due to PCMDI being down
	# for many, many months...
	fn_split = os.path.dirname( matches[0] ).split( os.path.sep )
	fn_test = [ i for i in fn_split if i.startswith( 'v' ) and i[1:].isdigit() ] # test and grab the one that has a v in it
	if len( fn_test ) != 0:
	...
	else: # this is for the nonstandard cloud inputs location where there are no version numbers
		grouped = matches.groupby( matches.apply( lambda x: os.path.basename(x).split( '_' )[3] ) ) #[HARDWIRED!]
		final_out = { i:j for i,j in grouped }
	return final_out
...
if __name__ == '__main__':
	...
	parser.add_argument( "-ip", "--input_dir", action="store", dest="input_dir", type=str, help="path to parent directory with a subdirector(ies)y storing the data" )
	parser.add_argument( "-op", "--output_dir", action="store", dest="output_dir", type=str, help="path to an output parent directory that will populate with output subdirector(ies)y." )
	...
	# output_dir = os.path.join( input_dir, 'prepped_november' )
	# if not os.path.exists( output_dir ):
	# 	os.makedirs( output_dir )

	# start an error log for any problem files
	problem_files_log = open( os.path.join( output_dir, 'error_files.txt' ), mode='w' )

	# make some filters and filter the files we want into groups
	fn_fn_prefix_filter_filter = variable + '_*' + model + '*'
	file_groups = group_input_filenames( fn_fn_prefix_filter_filter, os.path.join( input_dir, 'cmip5' ) )
	...
	for files in file_groups.values():
		try:
			...
			output_dir = os.path.join( output_dir, model, variable )
			...
			output_filename = os.path.join( output_dir, new_fn_base )
...
# variables = [ 'tas', 'hur' ]
# input_dir = '/workspace/Shared/Tech_Projects/ESGF_Data_Access/project_data/data/cmip5'
# output_dir = '/workspace/Shared/Tech_Projects/ESGF_Data_Access/project_data/data/prepped_november' # base prepped parent dir
# os.chdir( '/workspace/Shared/Tech_Projects/ALFRESCO_Inputs/project_data/CODE/tem_ar5_inputs' )
# for model in models:
# 	for variable in variables:
# 		os.system( ' '.join(['ipython --','ar5_model_data_preprocess.py','-ip', input_dir, '-op', output_dir, '-m', model, '-v', variable]) )


# # # # # CLOUDS PREP since it is a nonstandard download and storage location -- PCMDI down # # # # # # # # # # # # # # # # # # 
# import os
# models = [ 'GISS-E2-R', 'IPSL-CM5A-LR', 'MRI-CGCM3', 'CCSM4', 'GFDL-CM3' ]
# variables = [ 'clt' ]
# input_dir = '/workspace/Shared/Tech_Projects/ESGF_Data_Access/project_data/data/cmip5_clt_nonstandard'
# output_dir = '/workspace/Shared/Tech_Projects/ESGF_Data_Access/project_data/data/prepped_november' # base prepped parent dir
# os.chdir( '/workspace/Shared/Tech_Projects/ALFRESCO_Inputs/project_data/CODE/tem_ar5_inputs' )
# for model in models:
# 	for variable in variables:
