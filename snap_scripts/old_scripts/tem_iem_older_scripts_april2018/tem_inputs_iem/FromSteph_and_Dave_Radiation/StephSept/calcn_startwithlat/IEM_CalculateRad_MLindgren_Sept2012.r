...
setwd('/workspace/UA/malindgren/projects/iem/PHASE2_DATA/RSDS_working/September2012_finalRuns/outputs')
...
tmp = raster("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/Elevation/akcanada_prism_aspect_1km_MASTER.tif")
...
for (m in 1:12) {  # month loop
    ...
    rm(j)
       # Write the output data to a geotiff
        writeRaster(N,filename=paste("rad_1km_AKCanada_",month[m],'.tif',sep=''),format = 'GTiff',options='COMPRESS=LZW',datatype='FLT4S',overwrite=T)
        rm(N)
