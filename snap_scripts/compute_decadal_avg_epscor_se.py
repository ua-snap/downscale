...
def make_decadals( base_path, output_path, variable, model, scenario, decade, ncpus=32 ):
	'''
	...
	for month in month_grouped:
		...
		# THIS IS THE HARDWIRED METRIC of MEAN
		metric = 'mean'

		# this derives a mean from 3D (time, x, y) to 2D (x, y)
		mean_arr = np.mean( arr, axis=0 )
		decade_out = str(decade_begin)[:3] + '0s'
		output_filename = os.path.join( output_path, model, scenario, variable, '_'.join([var, metric, units, project, model, scenario, month, decade_out]) + '.tif' )
