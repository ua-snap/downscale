...
def f( x, mask ):
	...
	variable, metric, units, project, model, scenario, month, year = os.path.basename(x).split( '.' )[ 0 ].split( '_' )
	...
	masked = np.ma.masked_array( arr, mask == 0 )
	return '_'.join([model,scenario,domain,month,year]){ 'stdev':str( np.std( masked ) ),
...
# def max_of_mean( x, time_axis=0 ):
# 	return np.max( np.mean( x, axis=time_axis ) )
# def min_of_mean( x, time_axis=0 ):
# 	return np.max( np.mean( x, axis=time_axis ) )
# def mean_of_mean( x, time_axis=0 ):
# 	return np.mean( np.mean( x, axis=time_axis ) )
# def std_of_mean( x, time_axis=0 ):
# 	return np.std( np.mean( x, axis=time_axis ) )

def get_metrics( base_path, variable, model, scenario, begin, end, mask, domain_name=None, ncpus=32 ):
	'''
	...
	args = [ (i, mask) for i in modeled_files ]
...
if __name__ == '__main__':
	...
	begin_out = 1900
	end_out = 2100
	template_rst = '/workspace/Shared/Tech_Projects/EPSCoR_Southcentral/project_data/downscaled_cmip5_clipped/CCSM4/historical/tasmax/tasmax_mean_C_ar5_CCSM4_historical_1_1901.tif'
	...
	for variable in variables:
		...
		for model, scenario in itertools.product( models, scenarios ):
			...
			for year in range(begin, end+1):
				print( 'running: {} {} {} {}'.format( model, variable, scenario, year ) )
...
# 				# some output naming stuff for the dict key
# 				domain_num, = np.unique( mask[ mask > 0 ] )
# 				domain_name = subdomains.names_dict[ domain_num ].replace( ' ', '' )
# 				# place in a dict 
# 				all_data[ '_'.join([ model, scenario, variable, domain_name, str(decade_begin), str(decade_end) ]) ] = month_dict
# 		#cleanup
# 		del arr
...
# 	prefix = '_'.join([ variable, 'all_models', for_prefix, 'DECADALS', 'means_of_metrics', str(begin), str(end) ])
# 	# prefix = '_'.join([ variable, 'all_models', 'rcps', 'DECADALS', 'means_of_metrics', str(begin), str(end) ])
...
# 	# now some panel-y stuff with the output JSON
# 	panel = pd.Panel( deepcopy( all_data ) )
# 	metrics = ['mean','max','min','stdev']
# 	for metric in metrics:	
# 		df = panel[ :, metric, : ].T
# 		df = df[ [ str(i) for i in range(1,12+1) ] ] # sort the months
