...
if __name__ == '__main__':
	...
	for variable, model, scenario, decade in itertools.product( variables, models, scenarios, decades ):
		...
		for month in month_grouped:
			...
			pairs = zip( baseline, modeled )

			# get diffs in parallel
			pool = mp.Pool( ncpus )
			arr = np.array( pool.map( wrap_diff, pairs ) )
			pool.close()
			pool.join()
			pool.terminate()
			pool = None
		...
		prefix = '_'.join([ variable, 'all_models', for_prefix, 'DECADALS', 'DIFF','means_of_metrics', str(begin), str(end) ])
